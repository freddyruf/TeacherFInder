...
from __datafunction import create_connection
...
#parte da una frase  e cerca nella lista
def mostSimilarFromList(string, list):
    words = string.upper().split()  # divide the string into a list of words
    n = len(words)
    closest = None
    ...
    for i in range(n - 1):  # for each word in the list
        ...
        for element in list:  # for each row in the list
            ...
            for n in range(0,2): #cosi controllo l'ordine di nome/cognome
                if(n==1):
                    nomediviso=element.split()
                    element=nomediviso[1]+" "+nomediviso[0]
                d = similarity(element, substring)

                if d > min_distance and d > 75:
                    min_distance = d
                    if (n == 1):#nel caso trovo una somiglianza invertendo l'ordine, rimetto tutto a posto prima di andare avanti
                        nomediviso = element.split()
                        closest = nomediviso[1] + " " + nomediviso[0]
...
def prof_info(msg, orario, giorno):  # function to get the info about the professor
    ...
    Ora = Ora - 7  # -7 beacouse we use to talk about (frist, second, third...) hour and not about the actual hour for the clock
    print("2:",Ora)
...
def FindTime(str):  # function to find the hour in the string
    ...
    for c in str:  # for each word in the string in
        ...
        if (c == "prima"):  # if the word is "prima" the hour is 1
            ...
            return 15
        elif (c == "nona"):  # if the word is "nona" the hour is 9
            return 16
        elif (c == "decima"):  # if the word is "decima" the hour is 10
            return 17
