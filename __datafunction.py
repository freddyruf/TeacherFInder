...
def addLog(connection,cursor,message,feedmessage,feedresponse,feedbot):  # function to add the log element into the database
    ...
    if not isinstance(feedbot,str): #is we passed the pyrogram "Message" object and n ot a string
        feedbot=feedbot.text


    if message.text == polliceInSu: #search if we recive a feedback now, if yes we use NULL as "no feedback"
        message=1
    elif message.text == polliceInGiù:
        message=0
    else:
        message="NULL"

    cursor.execute(
                f"INSERT INTO log (tempo, resultmessage, requestinfo, usertext, feedback) VALUES ('{timeinfo}', '{feedmessage}','{feedresponse}', '{feedbot}', {message});")
